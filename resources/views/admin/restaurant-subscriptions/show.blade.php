@extends('admin.layout')

@section('title', 'Subscription Details')

@section('content')

    {{-- ================= PAGE HEADER ================= --}}
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold mb-1">
                Subscription #{{ $subscription->id }}
            </h3>
            <p class="text-muted mb-0">
                View subscription details and status
            </p>
        </div>

        <div class="d-flex gap-2">
            <a href="{{ route('admin.restaurant-subscriptions.edit', $subscription->id) }}"
                class="btn btn-outline-warning btn-sm">
                <i class="bi bi-pencil"></i> Edit
            </a>

            <a href="{{ route('admin.restaurant-subscriptions.index') }}" class="btn btn-outline-warning btn-sm">
                ← Back
            </a>
        </div>
    </div>

    {{-- Success --}}
    @if (session('success'))
        <div class="alert alert-success mb-4">
            {{ session('success') }}
        </div>
    @endif

    {{-- ================= CARD ================= --}}
    <div class="card">
        <div class="card-body p-4">

            <dl class="row mb-4">

                <dt class="col-sm-3 text-muted">Subscription ID</dt>
                <dd class="col-sm-9 fw-semibold">
                    #{{ $subscription->id }}
                </dd>

                <dt class="col-sm-3 text-muted">Restaurant</dt>
                <dd class="col-sm-9 fw-semibold">
                    {{ $subscription->restaurant->name ?? '—' }}
                </dd>

                <dt class="col-sm-3 text-muted">Plan</dt>
                <dd class="col-sm-9">
                    {{ $subscription->plan->name ?? ($subscription->subscriptionPlan->name ?? '—') }}
                </dd>

                <dt class="col-sm-3 text-muted">Status</dt>
                <dd class="col-sm-9">
                    @if ($subscription->is_active)
                        <span class="badge bg-success-subtle text-success">
                            Active
                        </span>
                    @else
                        <span class="badge bg-secondary-subtle text-secondary">
                            Inactive
                        </span>
                    @endif
                </dd>

                <dt class="col-sm-3 text-muted">Starts At</dt>
                <dd class="col-sm-9 text-muted">
                    {{ optional($subscription->starts_at)->format('Y-m-d') ?? '—' }}
                </dd>

                <dt class="col-sm-3 text-muted">Ends At</dt>
                <dd class="col-sm-9 text-muted">
                    {{ optional($subscription->ends_at)->format('Y-m-d') ?? '—' }}
                </dd>

                <dt class="col-sm-3 text-muted">Created At</dt>
                <dd class="col-sm-9 text-muted">
                    {{ $subscription->created_at->format('Y-m-d H:i') }}
                </dd>

            </dl>

            {{-- ================= ACTION ================= --}}
            <form action="{{ route('admin.restaurant-subscriptions.toggle', $subscription->id) }}" method="POST">
                @csrf
                <button class="btn btn-outline-warning">
                    {{ $subscription->is_active ? 'Deactivate Subscription' : 'Activate Subscription' }}
                </button>
            </form>

        </div>
    </div>

@endsection
